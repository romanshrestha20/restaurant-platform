...
-- DropIndex
DROP INDEX "profiles_fullName_idx";

-- Preserve the existing name while moving to the split-name profile shape.
ALTER TABLE "profiles" RENAME COLUMN "fullName" TO "firstName";

-- AlterTable
ALTER TABLE "profiles"
DROP COLUMN "helpCount",
DROP COLUMN "rating",
DROP COLUMN "searchRadiusMeters",
...
-- Preserve verification state and the legacy role long enough to backfill RBAC.
ALTER TABLE "users" RENAME COLUMN "isEmailVerified" TO "emailVerified";
ALTER TABLE "users" RENAME COLUMN "isPhoneVerified" TO "phoneVerified";
ALTER TABLE "users" RENAME COLUMN "role" TO "legacyRole";

UPDATE "users"
SET "emailVerified" = "emailVerified" OR "isVerified";
...
-- Seed global and restaurant-scoped roles, then retain legacy assignments.
...
FROM (VALUES
    ...
    ('CUSTOMER'),
    ('STAFF')
) AS seeded_roles("name");

INSERT INTO "user_roles" ("userId", "roleId", "assignedAt")
SELECT
    "id",
    'role_' || lower("legacyRole"::text),
    CURRENT_TIMESTAMP
FROM "users";
...
-- Move legacy profile avatars into reusable media and link them to users.
INSERT INTO "media" ("id", "url", "publicId", "createdAt")
SELECT
    'legacy_' || md5("userId" || "avatarUrl"),
    "avatarUrl",
    COALESCE("avatarPublicId", 'legacy_' || md5("userId" || "avatarUrl")),
    "createdAt"
FROM "profiles"
WHERE "avatarUrl" IS NOT NULL;

INSERT INTO "user_media" ("userId", "mediaId", "sortOrder", "isPrimary")
SELECT
    "userId",
    'legacy_' || md5("userId" || "avatarUrl"),
    0,
