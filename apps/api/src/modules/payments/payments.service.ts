...
export class PaymentsService {
  constructor(private readonly prisma: PrismaService, private readonly provider: NoopPaymentProvider) {}
  ...
  async retry(userId: string, orderId: string) {
    ...
    const created = await this.provider.createPayment({ amount: Number(payment.amount), currency: payment.currency, orderId, attemptId: attempt.id });
    ...
    return { paymentId: payment.id, attemptId: attempt.id, status: PaymentStatus.PROCESSING, checkoutUrl: created.checkoutUrl ?? null };
  }
