    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="text-center max-w-2xl mx-auto mb-12">
        ...
        <p className="mt-3 text-base sm:text-lg text-muted-foreground">
          Explore storefronts, browse handcrafted menus, and order directly from
          local favorites.
        </p>
      </div>
        ...
        <div className="p-5 flex-1 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-2">
              ...
              {address?.city && (
                <Badge variant="secondary">{address.city}</Badge>
              )}
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            <span>
              Prep: ~{restaurant.settings?.estimatedPrepMinutes ?? 30}m
            </span>
            <span>
              Delivery: €
              {Number(restaurant.settings?.deliveryFee ?? 0).toFixed(2)}
            </span>
